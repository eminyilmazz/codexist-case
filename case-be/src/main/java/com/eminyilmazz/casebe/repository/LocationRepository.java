...
public interface LocationRepository extends JpaRepository<Location, Long> {
    ...
    boolean existsByLatitudeAndLongitudeAndRadius(@Param("lat") double latitude, @Param("long") double longitude, @Param("rad") int radius);

    @Query("select l from Location l where l.latitude = :lat and l.longitude = :long and l.radius = :rad")
    Location findByLongitudeAndLongitudeAndRadius(@Param("lat") double latitude, @Param("long") double longitude, @Param("rad") int radius);
}
